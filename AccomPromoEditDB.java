...
public class AccomPromoEditDB extends HttpServlet{ 
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
      throws ServletException, IOException{
		...
		String title = "Edit Accomodation Details";
		...
		out.println(
         ...
         try{
            ...
            // Execute SQL query
            String sql = "UPDATE Accommodation SET prom_priority = "+prom_pri+ " WHERE accom_id= '" + aid + "'";
		    PreparedStatement stmt = conn.prepareStatement(sql);
			stmt.execute(sql);
			stmt.close();

			Statement st = conn.createStatement();
			String sql1 = "SELECT * FROM Accommodation WHERE accom_id= '" + aid + "'";
			ResultSet rs = st.executeQuery(sql1);
            out.print("<table>");
			out.print("<tr><th>Accomodation ID</th>");
